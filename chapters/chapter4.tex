...
本章样品均来自基于磁控溅射预制 \ce{Al2O3}/Fe 催化层的管式炉热 CVD 生长体系。制备时，先在 Si 基底表面磁控溅射氧化铝支撑层，再沉积不同功率和厚度条件下的 Fe 催化层。随后将样片置于管式炉中心，在 Ar/\ce{H2} 气氛下完成升温和退火预处理，并在设定生长温度下引入 \ce{C2H4} 作为碳源进行生长，获得具有不同取向和弯曲状态的碳纳米管阵列样品。生长完成后，将阵列侧面竖直固定于样品台上，通过 SEM 采集侧视图像用于后续形貌分析。为保证后续样品级比较的可比性，这里主要保留基本生长窗口相对一致条件下形成的样品差异。
...
本文的方法由弱标签构建、分割主干和训练约束三部分组成。参考已有方向引导弱监督分割思路\cite{Nguyen2023CNTSegNet}，本文采用自适应高斯阈值生成分割监督信号，以降低精细像素级标注成本。具体做法是，先对去除底部信息条后的主体区域进行限制对比度自适应直方图均衡化（CLAHE）增强和高斯平滑\cite{Zuiderveld1994CLAHE}，再采用自适应高斯阈值得到二值弱标签，并与主体区域图像同步裁剪为固定尺寸图块，用于模型训练。在此基础上，本文在高分辨率适配的 ResNet34-UNet 主干上引入方向分布约束和 clDice 连通性约束，构建 CNTSegNet-clDice 前景分割框架，如图~\ref{fig:4_4_segmentation_framework} 所示。
...
在获得稳定的碳纳米管二值前景图后，本文进一步提取样品级形貌特征。覆盖密度、取向度和表观直径沿原始几何路径计算，平均曲率、波曲度和迂曲度沿拓扑清理路径计算。图~\ref{fig:4_5_feature_pipeline} 给出了弯曲相关特征提取中的关键中间结果。
...
在比较前景结果后，再考察三类结果是否会影响后续弯曲特征的人工分级一致性。
...
最后比较人工粗筛与自动分级的处理效率和结果一致性。

本文以表现最稳定的迂曲度分级结果作为自动方法代表，并与人工粗筛结果进行对比。该自动分级准确率对应前述 CNTSegNet-clDice 前景输入下迂曲度单指标分级结果的 Accuracy。表~\ref{tab:4_8_efficiency_comparison} 汇总了两种方法在同一批 150 张样品图像上的总耗时、平均耗时和准确率，其中准确率均以人工三级标注结果作为参考标签。这里的自动耗时按单张图像顺序执行自动化表征与基于迂曲度的分级流程统计，不计人工复核与结果整理时间。结果显示，人工粗筛总耗时为 1960\,s，平均约 13.07\,s/张，准确率为 67.3\%；基于迂曲度的自动分级总耗时为 1548\,s，平均约 10.32\,s/张，准确率为 91.33\%。在当前实现条件下，自动方法既保持了较高的一致性，也在批量样品处理时表现出一定的效率优势。
...
在获得样品级形貌特征后，后续分析需要先处理变量范围问题。原始工艺记录覆盖了催化层预制、退火预处理和生长阶段的多项条件，若将 \ce{Al2O3} 参数、Fe 参数、气氛条件、温度和时间等变量同时展开，不仅会削弱分析重点，也会降低样品比较和结果讨论的清晰度。

因此，本文在保持背景生长窗口基本一致的前提下，将后续分析优先收敛到 Fe 功率、Fe 厚度和退火时间三项变量。这个收敛同时依据已有样品数据、样品可比性要求和后续关系建模需求，并参考第三章知识库支持下的工艺分析任务及 TCCER 返回的证据链。图中所示的归纳结果是在证据链基础上整理得到，用于说明为何保留这三项变量。变量保留并不只取决于统计显著性，还结合了它们与关键形貌差异的对应关系、相关工艺—机理—形貌证据链的完整性，以及后续关系建模与结果解释的需要。
...
仅凭相关性热图、分组趋势和响应分布，还不足以回答 CVD 制备条件变化能否稳定映射到阵列形貌变化上。对于碳纳米管阵列而言，工艺参数之间往往存在耦合作用，同一工艺条件下不同视野也可能表现出明显差异，弯曲相关特征还更容易受到局部几何扰动和图像噪声影响，因此关系分析不能只停留在趋势判断层面。为此，本文采用随机森林回归模型\cite{Breiman2001RandomForests}检验工艺参数与关键形貌特征之间是否存在可学习的非线性映射，并在建模结果基础上结合知识证据讨论变量作用。模型输入包括 Fe 功率、Fe 厚度、退火时间及其派生交互特征。对数值特征采用中位数填补并标准化处理，对分类型特征采用独热编码。模型评估采用按工艺组合分组的 5 折交叉验证，即每次用其中 4 折训练、1 折验证，并轮流完成全部分组的预测，以降低同源样本泄漏带来的偏高风险。这里引入该模型的目的不在于追求最优预测精度，而在于检验工艺参数与关键形貌特征之间是否存在可识别的非线性关系。模型性能通过决定系数（$R^2$）、均方根误差（RMSE）和平均绝对误差（MAE）进行综合评价，其主要超参数设置见表~\ref{tab:4_9_rf_settings}。
...
结合变量重要性与交叉验证结果可以看出，退火时间是四个形貌特征中影响更稳定的变量，Fe 厚度与 Fe 功率则更多体现为催化剂相关条件下的耦合调节变量。这说明前一小节基于统计结果得到的变量角色判断，能够在非线性建模中得到进一步支持。为说明这种变量分工背后的材料过程基础，下面结合第三章构建的知识库、工艺分析任务以及 TCCER 返回的证据链，对已经观察到的统计关系和建模结果作进一步解释。相关知识增强结果只用于解释变量作用，不参与变量筛选、显著性判断和模型训练。
...
综合统计结果、随机森林建模和证据链解释，在背景生长窗口基本一致的前提下，Fe 功率、Fe 厚度和退火时间可优先纳入工艺—形貌关系分析。Fe 厚度与 Fe 功率虽然在单变量层面不表现为主导整体趋势的连续变化，但在催化层形成阶段具有较明确的耦合调节意义，更适合作为耦合调节变量理解。退火时间通过影响催化剂形成与演化窗口，进一步作用于阵列整体有序性和弯曲相关特征，因此可作为解释取向变化和弯曲增强趋势的重要变量。自动化形貌表征提供了样品级指标，知识检索结果补充了变量收敛和结果解释所需依据，本章据此完成了工艺—形貌关系的主要分析。

\section{小结}
\ensection{Summary}

本章说明了碳纳米管阵列样品与图像数据来源，构建了面向样品级比较的自动化形貌表征流程，并在此基础上开展了工艺—形貌关系分析。结果表明，SEM 图像可以转化为用于统计分析和关系建模的样品级形貌指标，知识检索结果补充了变量筛选和结果解释所需依据。本章据此完成了工艺—形貌关系分析。
